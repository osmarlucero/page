	...
	<BODY>
		<div>
			...
			<?php if (isset($_SESSION) && isset($_SESSION['error'])): ?>
					<h3>Error: <?=$_SESSION['error'] ?></h3>
					<?php unset($_SESSION['error'])?>
			<?php endif ?>
			<?php if (isset($_SESSION) && isset($_SESSION['success'])): ?>
					<h3>COrrecto: <?=$_SESSION['success'] ?></h3>
				...
				<tbody>
						<?php foreach ($categories as $category): ?>
								...
								<tr>
									...
									<td>
										<button onclick="edit(<?= $category['id'] ?>,'<?= $category['name'] ?>',
									'<?= $category['description'] ?>', '<?= $category['status'] ?>')">edit</button>
										<button onclick="deleteMovie(<?= $category['id'] ?>)">DELETE</button>
								</td>
		...
		<script type="text/javascript">
			function edit(id,name, description,status){
				...
				alert("hola "+id+name+description+status);
			}
			function deleteMovie(id){
				var conf=prompt("Desea eliminar, escriba:"+id);
				if(conf== id){
